...
analyse_sentiment <- function(datafile, country.choice, ndays, num.bootstraps, outpref)
{
  ...
  p <- ggplot(subset.df, aes(x = Date, y = Positive - Negative)) + geom_path(linewidth=0.7) + theme_light() + theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 12), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 11), legend.title=element_text(size=14,face="bold"), legend.text=element_text(size=12), legend.position = "none") + ylab(paste("No. Positive - No. Negative Tweets", sep = "")) + scale_fill_npg()
  p
  ggsave(file=paste(outpref, "_pos_v_neg.svg", sep = ""), plot=p, height = 8, width = 15)
  
  p <- ggplot(subset.df, aes(x = Date, y = Prop.negative)) + geom_path(linewidth=0.7) + theme_light() + theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 12), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 11), legend.title=element_text(size=14,face="bold"), legend.text=element_text(size=12), legend.position = "none") + ylab(paste("Proportion Negative Tweets", sep = "")) + scale_fill_npg()
  p
  ggsave(file=paste(outpref, "_prop_neg.svg", sep = ""), plot=p, height = 8, width = 15)
}
